...
const ContactAgentForm = () => {
  ...
  const [msgByUser, setMsgByUser] = useState(null);
  ...
  const postMessage = async (e) => {
    ...
    try {
      const response = await postData( `${process.env.REACT_APP_PROXY}/api/message`, data);
  ...
  <form action="" className="form-contact-agent">
    ...
    <div className="form-group">
      <input onClick={postMessage}
        ...
        value = {msgByUser} onChange = {(e) =>setMsgByUser(e.target.value)}
      />
    </div>
  </form>
  {msgError && (
